...
def fuzz(requests_data: RequestData, payloads: set[str], canary: str):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for _ in range(0, len(payloads)):
            futures.append(
                executor.submit(
                    does_payload_execute, session, requests_data, payloads.pop(), canary
                )
            )
        vulnerable = False
...
def main():
    ...
    arg_parser.add_argument(
        "--http-proxy", required=False, help="HTTP proxy to use", default=""
    )
    arg_parser.add_argument(
        "--https-proxy", required=False, help="HTTPS proxy to use", default=""
    )
